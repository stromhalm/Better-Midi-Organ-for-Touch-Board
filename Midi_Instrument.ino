int instrument = 47; // Specify the instrument number (see the list below)
int pitch = 0; // Specify the pitch (0 is "normal", >0 for higher, <0 for lower).
int volume = 127; // Specify the volume (0 for muted, 127 for max).
boolean whiteNotesOnly = false; // Type true to use only white notes (but pitch has to be a multiple of 7)
...
void loop(){
   if(MPR121.touchStatusChanged()){
     ...
     for(int i=firstPin; i<=lastPin; i++){
       ...
       if(whiteNotesOnly) {
         note = whiteNotes[lastPin-i]+pitch;
       } else {
        note = allNotes[lastPin-i]+pitch;
       }
...
void setupMidi(){
  ...
  talkMIDI(0xB0, 0x07, volume); //0xB0 is channel message, set channel volume to max (127)
